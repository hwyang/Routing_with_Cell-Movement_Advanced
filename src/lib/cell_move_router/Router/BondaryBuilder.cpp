...
void BondaryBuilder::createBondaryInfo() {
  ...
  MinR = std::max(MinR - 6, RowBeginIdx);
  MaxR = std::min(MaxR + 6, RowEndIdx);
  MinC = std::max(MinC - 6, ColBeginIdx);
  MaxC = std::min(MaxC + 6, ColEndIdx);
  MinL = std::max(MinL - 2, MinRoutingLayConstraint->getIdx());
  MaxL = std::min(MaxL + 2, LayerSz);
